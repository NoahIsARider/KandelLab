...
export default function LateralInhibitionPage() {
  ...
  return (
    <article>
      <ChapterHeading>§7 侧抑制 — 中心-周围拮抗与边缘增强</ChapterHeading>
      ...
      <p className="text-sm text-[var(--ink)] mb-4 leading-relaxed">
        Difference-of-Gaussians (DOG) 模型描述感受野的中心-周围拮抗结构。
        均匀区域响应平坦，边缘处出现增强（Mach band 效应），
        这是视觉系统增强对比度的基本机制。
      </p>
      ...
      <ResultCard title="参数">
        ...
        <div className="flex items-center gap-3 mt-2">
          <label className="text-sm text-[var(--ink)] font-[var(--font-display)] min-w-[120px]">刺激类型</label>
          <select
            value={stimType}
          ...
          >
            ...
            <option value="edge">阶跃边缘</option>
            <option value="gradient">线性梯度</option>
          </select>
        </div>
        <div className="mt-4">
          <RunButton onRun={run} />
      ...
      {result && (
        <>
          <ResultCard title="刺激与响应">
            <Waveform data={result.stimulus} height={60} color="var(--ink-light)" label="刺激" />
            <Waveform data={result.response} height={60} color="var(--oxide)" label="响应（边缘增强）" />
          </ResultCard>

          <ResultCard title="DOG 核">
            <Waveform data={result.kernel} height={50} color="var(--verdigris)" label="1D DOG 核" />
          </ResultCard>
        </>
      )}

      {kernel2D.length > 0 && (
        <ResultCard title="2D DOG 感受野热图">
